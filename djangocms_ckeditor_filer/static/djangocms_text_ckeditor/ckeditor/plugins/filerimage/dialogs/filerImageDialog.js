...
CKEDITOR.dialog.add('filerImageDialog', function(editor) {
	...
	function getImageUrl() {
		var url = dialog.getContentElement("tab-basic", "url"),
			...
			width = 0,
			height = 0,
			image_id = jQuery('#id_image').val() ;

		if (thumb_sel_val != 0) {
			server_url = '?djangocms_ckeditor_filer_image='+ image_id + '&thumb_options=' + thumb_sel_val;
		} else {
			width = dialog.getContentElement("tab-basic", "width").getValue();
			height = dialog.getContentElement("tab-basic", "height").getValue();
			server_url = '?djangocms_ckeditor_filer_image='+ image_id + '&width=' + width + '&height=' + height;
		}
